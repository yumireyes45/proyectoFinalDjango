...
class TestQAPOnce:
    ...
    def test_common_input_validation(self):
        ...
        with pytest.raises(ValueError, match="`A` must be square"):
            quadratic_assignment(
                np.random.random((3, 4)), np.random.random((3, 3)),
            )
        with pytest.raises(ValueError, match="`B` must be square"):
            quadratic_assignment(
                np.random.random((3, 3)), np.random.random((3, 4)),
            )
        ...
        with pytest.raises(ValueError, match="`A` and `B` must have exactly two"):
            quadratic_assignment(
                np.random.random((3, 3, 3)), np.random.random((3, 3, 3)),
            )
        ...
        with pytest.raises(
            ...
            quadratic_assignment(
                np.random.random((3, 3)), np.random.random((4, 4)),
            )
        # can't have more seed nodes than cost/dist nodes
        _rm = _range_matrix
        with pytest.raises(
