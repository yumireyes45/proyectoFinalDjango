...
def minimize(
    ...
    **Custom minimizers**
    ...
    if meth not in ("cobyla", "slsqp", "trust-constr", "_custom") and np.any(
        constraints
    ):
        warn(
            "Method %s cannot handle constraints." % method,
            RuntimeWarning,
            stacklevel=2,
        )
    if (
        ...
        warn("Method %s cannot handle bounds." % method, RuntimeWarning, stacklevel=2)
    # - return_all
    if meth in ("l-bfgs-b", "tnc", "cobyla", "slsqp") and options.get(
        "return_all", False
    ):
        warn(
            "Method %s does not support the return_all option." % method,
            RuntimeWarning,
            stacklevel=2,
        )
