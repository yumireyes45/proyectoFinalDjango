...
class SVDSCommonTests:
    ...
    def test_svd_maxiter(self):
        ...
        ud, sd, vhd = svds(A, k, solver=self.solver, maxiter=maxiter, random_state=0)
    ...
    def test_svd_linop(self):
        ...
        for n, m, k in nmks:
            ...
            if solver == "lobpcg":
                with pytest.warns(UserWarning, match="The problem size"):
                    U1, s1, VH1 = reorder(
                        svds(A, k, v0=v0, solver=solver, random_state=0)
                    )
                    U2, s2, VH2 = reorder(
                        svds(L, k, v0=v0, solver=solver, random_state=0)
                    )
            else:
                U1, s1, VH1 = reorder(svds(A, k, v0=v0, solver=solver, random_state=0))
                U2, s2, VH2 = reorder(svds(L, k, v0=v0, solver=solver, random_state=0))
            ...
            if self.solver == "lobpcg":
                with pytest.warns(UserWarning, match="The problem size"):
                    U1, s1, VH1 = reorder(
                        svds(A, k, which="SM", solver=solver, random_state=0, **kwargs)
                    )
                    U2, s2, VH2 = reorder(
                        svds(L, k, which="SM", solver=solver, random_state=0, **kwargs)
                    )
            else:
                U1, s1, VH1 = reorder(
                    svds(A, k, which="SM", solver=solver, random_state=0, **kwargs)
            ...
            if k < min(n, m) - 1:
                ...
                for (dt, eps) in [(complex, 1e-7), (np.complex64, 3e-3)]:
                    ...
                    if self.solver == "lobpcg":
                        with pytest.warns(UserWarning, match="The problem size"):
                            U1, s1, VH1 = reorder(
                                svds(A, k, which="LM", solver=solver, random_state=0)
                            )
                            U2, s2, VH2 = reorder(
                                svds(L, k, which="LM", solver=solver, random_state=0)
                            )
                    else:
                        U1, s1, VH1 = reorder(
                            svds(A, k, which="LM", solver=solver, random_state=0)
    ...
    def test_small_sigma_sparse(self, shape, dtype):
        ...
        S = spdiags(e, 0, m, m) @ S
        S = S.astype(dtype)
        u, s, vh = svds(S, k, which="SM", solver=solver, maxiter=1000, random_state=0)
    ...
    def test_small_sigma2(self, dtype):
        ...
        su, ss, svh = svds(
            sp_mat, k=dim, which="SM", solver=self.solver, random_state=0
        )
