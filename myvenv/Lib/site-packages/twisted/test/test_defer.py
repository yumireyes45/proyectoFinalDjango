...
class DeferredTests(unittest.SynchronousTestCase, ImmediateFailureMixin):
    ...
    def test_maybeDeferredCoroutineFailure(self) -> None:
        ...
        assert_that(
            self.failureResultOf(defer.maybeDeferred(g)).type, equal_to(SomeException),
        )
    ...
    def test_circularChainException(self) -> None:
        ...
        """
        self.addCleanup(
            setattr, warnings, "filters", warnings.filters,
        )
    ...
    def test_reprWithChaining(self) -> None:
        ...
        b.chainDeferred(a)
        self.assertEqual(
            repr(a), f"<Deferred at 0x{id(a):x} waiting on Deferred at 0x{id(b):x}>",
        )
...
class RaceTests(unittest.SynchronousTestCase):
    ...
    def test_failure(self, beforeWinner: int, afterWinner: int) -> None:
        ...
        assert_that(
            actualFailure.value.failures, equal_to([failure] * len(ds)),
        )
        assert_that(
            cancelledState, equal_to([0] * len(ds)),
        )
...
class LogTests(unittest.SynchronousTestCase):
    ...
    def test_errorLogNoRepr(self) -> None:
        ...
        self.assertTrue(
            msg.startswith(expected), f"Expected message starting with: {expected!r}",
        )
