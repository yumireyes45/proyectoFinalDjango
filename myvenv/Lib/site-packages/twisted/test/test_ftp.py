...
class FTPServerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Start the server
        p = portal.Portal(
            ftp.FTPRealm(anonymousRoot=self.directory, userHome=self.directory,)
        )
...
class BasicFTPServerTests(FTPServerTestCase):
    ...
    def test_NotLoggedInReply(self):
        ...
        def checkPassResponse(result, command):
            ...
            self.assertFalse(
                ...
                "%s - Response start with 530: %r" % (command, result,),
            )
    ...
    def test_STORreturnsErrorFromOpen(self):
        ...
        self.assertCommandFailed(
            "STOR folder", ["550 folder: is a directory"], chainDeferred=d,
        )
        return d
    ...
    def test_unique(self):
        ...
        for key, value in vars(ftp).items():
            if isinstance(value, str) and key.isupper():
                ...
                self.assertNotIn(
                    value, seenValues, f"Duplicate code {key!r} with value {value!r}",
                )
