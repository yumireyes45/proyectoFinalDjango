    ...
    def getSynopsis(self) -> str:
        ...
        if self.parent is None:
            default = "Usage: {}{}".format(
                executableName, (self.longOpt and " [options]") or "",
            )
        else:
    ...
    def _shellCode(self, optName, shellType):
        if shellType == _ZSH:
            return '{}:{}:_files -g "{}"'.format(
                self._repeatFlag, self._description(optName), self._globPattern,
            )
        raise NotImplementedError(f"Unknown shellType {shellType!r}")
...
class CompleteList(Completer):
    ...
    def _shellCode(self, optName, shellType):
        if shellType == _ZSH:
            return "{}:{}:({})".format(
                self._repeatFlag, self._description(optName), " ".join(self._items,),
            )
        raise NotImplementedError(f"Unknown shellType {shellType!r}")
