...
class WorkerPoolTests(TestCase):
    ...
    def test_runUsedDirectory(self):
        ...
        self.assertEqual(
            started.workingDirectory, self.workingDirectory.sibling("_trial_temp-1"),
        )
    ...
    @given(
        booleans(), sampled_from(["out.log", f"subdir{sep}out.log",]),
    )
...
class DistTrialRunnerTests(TestCase):
    ...
    def test_exitFirst(self):
        ...
        result = self.successResultOf(d)
        assert_that(
            result.original, matches_result(successes=1, failures=has_length(1),),
        )
    ...
    def test_runUntilFailure(self):
        ...
        self.assertEqual(
            output.count("PASSED"), case.n - 1, "expected to see PASSED in output",
        )
