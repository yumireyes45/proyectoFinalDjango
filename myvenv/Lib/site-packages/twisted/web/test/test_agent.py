...
class HTTPConnectionPoolTests(TestCase, FakeReactorAndConnectMixin):
    ...
    def test_getCachedConnection(self):
        ...
        return self.pool.getConnection(
            ("http", b"example.com", 80), BadEndpoint(),
...
class AgentMethodInjectionTests(
    FakeReactorAndConnectMixin, MethodInjectionTestsMixin, SynchronousTestCase,
):
    """
...
class AgentURIInjectionTests(
    FakeReactorAndConnectMixin, URIInjectionTestsMixin, SynchronousTestCase,
):
    """
...
class CookieTestsMixin:
    ...
    def addCookies(
        ...
        response = client._FakeStdlibResponse(
            client.Response(
                (b"HTTP", 1, 1), 200, b"OK", Headers({b"Set-Cookie": cookies}), None,
            )
        )
...
class CookieAgentTests(
    ...
    def test_emptyCookieJarRequest(self):
        ...
        resp = client.Response(
            (b"HTTP", 1, 1), 200, b"OK", Headers({b"Set-Cookie": [b"foo=1",]}), None,
        )
        res.callback(resp)
...
class _RedirectAgentTestsMixin(testMixinClass):
    ...
    def _sensitiveHeadersTest(
        ...
        self.assertEquals(
            sameOriginHeaders, {b"host": [b"example.com"], **normHeaders(allHeaders),},
        )
...
class ReadBodyTests(TestCase):
    ...
    def test_withPotentialDataLoss(self):
        ...
        self.assertEqual(
            ...
            {"status": b"200", "message": b"OK", "body": b"firstsecond",},
        )
...
class RequestMethodInjectionTests(
    MethodInjectionTestsMixin, SynchronousTestCase,
):
    """
...
class RequestURIInjectionTests(
    URIInjectionTestsMixin, SynchronousTestCase,
):
    """
    ...
    def attemptRequestWithMaliciousURI(self, uri):
        ...
        """
        client.Request(
            method=b"GET", uri=uri, headers=http_headers.Headers(), bodyProducer=None,
        )
