...
class Lock:
    ...
    def __enter__(self) -> "Lock":
        if self.acquire():
            return self
        raise LockError(
            "Unable to acquire lock within the time specified", lock_name=self.name,
        )
    ...
    def do_release(self, expected_token: str) -> None:
        if not bool(
            ...
            raise LockNotOwnedError(
                "Cannot release a lock that's no longer owned", lock_name=self.name,
            )
    ...
    def do_extend(self, additional_time: int, replace_ttl: bool) -> bool:
        ...
        if not bool(
            ...
            raise LockNotOwnedError(
                "Cannot extend a lock that's no longer owned", lock_name=self.name,
            )
        return True
    ...
    def do_reacquire(self) -> bool:
        ...
        if not bool(
            ...
            raise LockNotOwnedError(
                "Cannot reacquire a lock that's no longer owned", lock_name=self.name,
            )
        return True
