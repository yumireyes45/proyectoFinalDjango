...
def ndarray_to_mgr(
    ...
    if dtype is not None and values.dtype != dtype:
        ...
        values = sanitize_array(
            values, None, dtype=dtype, copy=copy_on_sanitize, allow_2d=True,
        )
...
def _list_of_dict_to_arrays(
    data: list[dict], columns: Index | None,
) -> tuple[np.ndarray, Index]:
    """
...
def _finalize_columns_and_data(
    content: np.ndarray, columns: Index | None, dtype: DtypeObj | None,  # ndim == 2
) -> tuple[list[ArrayLike], Index]:
    """
