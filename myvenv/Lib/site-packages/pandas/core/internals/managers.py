...
class BaseBlockManager(DataManager):
    ...
    def setitem(self, indexer, value, warn: bool = True) -> Self:
        ...
        if warn and warn_copy_on_write() and not self._has_no_reference(0):
            warnings.warn(
                COW_WARNING_GENERAL_MSG, FutureWarning, stacklevel=find_stack_level(),
            )
        ...
        if self.is_single_block:
            ...
            if sl_type == "slice":
                ...
                if not allow_fill and only_slice:
                    ...
                    return [
                        blk.take_nd(
                            slobj, axis=0, new_mgr_locs=bp, fill_value=fill_value,
                        )
                    ]
    ...
    def take(
        self, indexer: npt.NDArray[np.intp], axis: AxisInt = 1, verify: bool = True,
    ) -> Self:
        """
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
        ...
        if lib.is_integer(loc):
            ...
            if len(blk._mgr_locs) == 1:  # TODO: fastest way to check this?
                return self._iset_single(
                    loc, value, inplace=inplace, blkno=blkno, blk=blk, refs=refs,
                )
        ...
        if self.is_single_block:
            ...
            if blk.is_extension:
                ...
                arr = blk.values.to_numpy(  # type: ignore[union-attr]
                    dtype=dtype, na_value=na_value, copy=copy,
                ).reshape(blk.shape)
            elif not copy:
    ...
    def _interleave(
        self, dtype: np.dtype | None = None, na_value: object = lib.no_default,
    ) -> np.ndarray:
        """
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    def __init__(
        self, block: Block, axis: Index, verify_integrity: bool = False,
    ) -> None:
...
def raise_construction_error(
    tot_items: int, block_shape: Shape, axes: list[Index], e: ValueError | None = None,
):
