...
class TestMerge:
    ...
    def test_index_and_on_parameters_confusion(self, df, df2):
        ...
        with pytest.raises(ValueError, match=msg):
            merge(
                df, df2, how="left", left_index=["key1", "key2"], right_index=False,
            )
        with pytest.raises(ValueError, match=msg):
            merge(
                df,
    ...
    def test_other_datetime_unit(self, unit):
        ...
        exp = DataFrame(
            {"entity_id": [101, 102], "days": days,}, columns=["entity_id", "days"],
        )
...
class TestMergeDtypes:
    ...
    @pytest.mark.parametrize(
        "expected_data, how",
        [([1, 2], "outer"), ([], "inner"), ([2], "right"), ([1], "left"),],
    )
    ...
    @pytest.mark.parametrize(
        "expected_data, how",
        [(["a", "b"], "outer"), ([], "inner"), (["b"], "right"), (["a"], "left"),],
    )
...
class TestMergeCategorical:
    ...
    def test_basic(self, left, right, using_infer_string):
        ...
        dtype = np.dtype("O") if not using_infer_string else "string"
        expected = Series(
            [CategoricalDtype(categories=["foo", "bar"]), dtype, np.dtype("int64"),],
            index=["X", "Y", "Z"],
        )
        tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize("func", ["merge", "merge_asof"])
@pytest.mark.parametrize(
    "kwargs", [{"right_index": True}, {"left_index": True},],
)
