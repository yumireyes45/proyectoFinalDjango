...
class TestAsOfMerge:
    ...
    def test_allow_exact_matches_and_tolerance2(self):
        ...
        result = merge_asof(
            df1, df2, on="time", allow_exact_matches=False, tolerance=Timedelta("10ms"),
        )
        expected = pd.DataFrame(
            {
                "time": to_datetime(["2016-07-15 13:30:00.030"]),
    ...
    def test_by_dtype(self, dtype):
        # GH 55453, GH 22794
        left = pd.DataFrame(
            {"by_col": np.array([1], dtype=dtype), "on_col": [2], "value": ["a"],}
        )
        right = pd.DataFrame(
            {"by_col": np.array([1], dtype=dtype), "on_col": [1], "value": ["b"],}
        )
        result = merge_asof(left, right, by="by_col", on="on_col")
        expected = pd.DataFrame(
            {
...
def test_merge_asof_extension_dtype(dtype):
    ...
    expected = pd.DataFrame(
        {"join_col": [1, 3, 5], "left_val": [1, 2, 3], "right_val": [np.nan, 2.0, 3.0],}
    )
