...
class TestPivotTable:
    ...
    def test_pivot_with_tz(self, method, unit):
        ...
        expected = DataFrame(
            [[0, 2, 0, 2], [1, 3, 1, 3]], index=exp_idx, columns=exp_col,
        )
    ...
    def test_pivot_datetime_tz(self):
        ...
        expected2 = DataFrame(
            np.array(
                [[0.0, 3.0, 1.0, 2.0], [1.0, 4.0, 2.0, 1.0], [2.0, 5.0, 1.0, 2.0],],
            ),
            index=exp_idx,
            columns=exp_col[4:],
        )
    ...
    def test_pivot_table_sort_false_with_multiple_values(self):
        ...
        expected = DataFrame(
            ...
            columns=["height", "age"],
            index=MultiIndex.from_tuples(
                [("Foo", "John"), ("Bar", "Michael")], names=["lastname", "firstname"],
            ),
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_table_aggfunc_nunique_with_different_values(self):
        ...
        columnval = MultiIndex.from_arrays(
            [["nunique" for i in range(10)], ["c" for i in range(10)], range(10),],
            names=(None, None, "b"),
        )
        ...
        result = test.pivot_table(
            index=["a",], columns=["b",], values=["c",], aggfunc=["nunique"],
        )
