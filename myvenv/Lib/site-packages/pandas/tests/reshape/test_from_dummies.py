...
def test_error_contains_non_dummies():
    ...
    with pytest.raises(
        TypeError, match=r"Passed DataFrame contains non-dummy data",
    ):
        from_dummies(dummies)
...
def test_error_with_prefix_multiple_seperators():
    ...
    with pytest.raises(
        ValueError, match=(r"Separator not specified for column: col2-a"),
    ):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "c", DataFrame({"": ["a", "b", "c"]}), id="default_category is a str",
        ),
        pytest.param(
            1, DataFrame({"": ["a", "b", 1]}), id="default_category is a int",
        ),
        pytest.param(
            1.25, DataFrame({"": ["a", "b", 1.25]}), id="default_category is a float",
        ),
        pytest.param(0, DataFrame({"": ["a", "b", 0]}), id="default_category is a 0",),
        pytest.param(
            False, DataFrame({"": ["a", "b", False]}), id="default_category is a bool",
        ),
        pytest.param(
            (1, 2),
            DataFrame({"": ["a", "b", (1, 2)]}),
