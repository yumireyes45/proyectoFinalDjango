...
class TestIndex:
    ...
    @pytest.mark.parametrize(
        "klass,dtype,na_val",
        [(Index, np.float64, np.nan), (DatetimeIndex, "datetime64[ns]", pd.NaT),],
    )
    ...
    @pytest.mark.parametrize(
        "klass", [Index, CategoricalIndex, DatetimeIndex, TimedeltaIndex,],
    )
    ...
    @pytest.mark.parametrize(
        "index", ["int64", "int32", "range"], indirect=True,
    )
    ...
    def test_isin_nan_common_object(
        ...
        if (
            ...
            tm.assert_numpy_array_equal(
                idx.isin([nulls_fixture2]), np.array([False, True]),
            )
            ...
            tm.assert_numpy_array_equal(
                idx.isin([nulls_fixture2]), np.array([False, False]),
            )
    ...
    @pytest.mark.parametrize(
        "labels,dtype", [(DatetimeIndex([]), np.datetime64),],
    )
