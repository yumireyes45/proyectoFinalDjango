...
class TestTimedeltaConstructorUnitKeyword:
    ...
    @pytest.mark.parametrize(
        ...
        + [
            (value, "m")
            for value in ["m", "minute", "min", "minutes", "Minute", "Min", "Minutes",]
        ]
        + [
            (value, "s")
            for value in ["s", "seconds", "sec", "second", "Seconds", "Sec", "Second",]
        ]
        + [
            (value, "ms")
            for value in [
    ...
    def test_unit_parser(self, unit, np_unit, wrapper):
        ...
        expected = TimedeltaIndex(
            [np.timedelta64(i, np_unit) for i in np.arange(5).tolist()], dtype="m8[ns]",
        )
