...
class TestDataFrameReplace:
    ...
    def test_replace_mixed2(self, using_infer_string):
        ...
        expected = DataFrame(
            {"A": Series(["foo", "bar"]), "B": Series([0, "foo"], dtype="object"),}
        )
    ...
    def test_replace_with_None_keeps_categorical(self):
        ...
        df = DataFrame(
            {"id": Series([5, 4, 3, 2], dtype="float64"), "col": cat_series,}
        )
        result = df.replace({3: None})

        expected = DataFrame(
            {"id": Series([5.0, 4.0, None, 2.0], dtype="object"), "col": cat_series,}
        )
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameReplaceRegex:
    ...
    def test_replace_categorical_no_replacement(self):
        # GH#46672
        df = DataFrame(
            {"a": ["one", "two", None, "three"], "b": ["one", None, "two", "three"],},
            dtype="category",
        )
        expected = df.copy()
