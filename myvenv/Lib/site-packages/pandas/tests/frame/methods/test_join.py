...
def test_join_on_single_col_dup_on_right(left_no_dup, right_w_dups, dtype):
    ...
    left_no_dup.join(
        right_w_dups, on="a", validate="one_to_many",
    )
    ...
    with pytest.raises(MergeError, match=msg):
        left_no_dup.join(
            right_w_dups, on="a", validate="one_to_one",
        )
...
def test_join_on_single_col_dup_on_left(left_w_dups, right_no_dup):
    ...
    left_w_dups.join(
        right_no_dup, on="a", validate="many_to_one",
    )
    ...
    with pytest.raises(MergeError, match=msg):
        left_w_dups.join(
            right_no_dup, on="a", validate="one_to_one",
        )
...
def test_join_on_single_col_dup_on_both(left_w_dups, right_w_dups):
    ...
    with pytest.raises(MergeError, match=msg):
        left_w_dups.join(
            right_w_dups, on="a", validate="many_to_one",
        )
