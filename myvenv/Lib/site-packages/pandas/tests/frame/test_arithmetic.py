...
class TestFrameFlexArithmetic:
    ...
    def test_frame_multiindex_operations(self):
        ...
        series = Series(
            [0.4], index=MultiIndex.from_product([["b"], ["a"]], names=["mod", "scen"]),
        )
    ...
    def test_frame_multiindex_operations_no_align(self):
        ...
        series = Series(
            [0.4], index=MultiIndex.from_product([["c"], ["a"]], names=["mod", "scen"]),
        )

        expected = DataFrame(
            {2010: np.nan, 2020: np.nan},
            index=MultiIndex.from_tuples(
                [("a", "b", 0), ("a", "b", 1), ("a", "b", 2), ("a", "c", np.nan),],
                names=["scen", "mod", "id"],
            ),
        )
        result = df.add(series, axis=0)
...
class TestFrameArithmeticUnsorted:
    ...
    @pytest.mark.parametrize(
        "val",
        [[1, 2, 3], (1, 2, 3), np.array([1, 2, 3], dtype=np.int64), range(1, 4),],
    )
