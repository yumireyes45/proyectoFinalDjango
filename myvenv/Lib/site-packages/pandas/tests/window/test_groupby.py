...
class TestRolling:
    ...
    def test_rolling_corr_cov_unordered(self, func, expected_values):
        # GH 43386
        df = DataFrame(
            {"a": ["g1", "g2", "g1", "g1"], "b": [0, 0, 1, 2], "c": [2, 0, 6, 4],}
        )
    ...
    def test_groupby_rolling_center_on(self):
        ...
        expected = Series(
            [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 7.0, 7.5, 7.5, 7.5], name="value", index=mi,
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_rolling_count_closed_on(self, unit):
        ...
        mi = MultiIndex.from_arrays(
            [["A", "A", "A", "B", "B", "B"], dti,], names=["group", "date"],
        )
        expected = Series(
            [np.nan, 1.0, 1.0, np.nan, 1.0, 1.0], name="column1", index=mi,
        )
        tm.assert_series_equal(result, expected)
