...
class TestOperations:
    ...
    def test_date_boolean(self, engine, parser):
        ...
        res = self.eval(
            "df.dates1 < 20130101", local_dict={"df": df}, engine=engine, parser=parser,
        )
    ...
    @pytest.mark.parametrize(
        "expr", ["df > 2 and df > 3", "df > 2 or df > 3", "not df > 2",],
    )
    def test_fails_and_or_not(self, expr, engine, parser):
        ...
        if parser == "python":
            ...
            with pytest.raises(NotImplementedError, match=msg):
                pd.eval(
                    expr, local_dict={"df": df}, parser=parser, engine=engine,
                )
        else:
...
class TestMath:
    ...
    def test_df_use_case(self, engine, parser):
        ...
        df.eval(
            "e = arctan2(sin(a), b)", engine=engine, parser=parser, inplace=True,
        )
        got = df.e
...
@td.skip_if_no("numexpr")
@pytest.mark.parametrize(
    ("use_numexpr", "expected"), ((True, "numexpr"), (False, "python"),),
)
