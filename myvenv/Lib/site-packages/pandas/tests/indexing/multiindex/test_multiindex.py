...
class TestMultiIndexBasic:
    ...
    def test_multiindex_complex(self):
        ...
        result = DataFrame(
            {"x": complex_data, "y": non_complex_data, "z": non_complex_data,}
        )
        result.set_index(["x", "y"], inplace=True)
        expected = DataFrame(
            {"z": non_complex_data},
            index=MultiIndex.from_arrays(
                [complex_data, non_complex_data], names=("x", "y"),
            ),
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_multiindex_with_na_missing_key(self):
        ...
        df = DataFrame.from_dict(
            {("foo",): [1, 2, 3], ("bar",): [5, 6, 7], (None,): [8, 9, 0],}
        )
    ...
    def test_multiindex_dtype_preservation(self):
        ...
        df = DataFrame(
            [[1, 0], [0, 1]],
            columns=[["foo", "foo"], ["location", "location"], ["x", "y"],],
