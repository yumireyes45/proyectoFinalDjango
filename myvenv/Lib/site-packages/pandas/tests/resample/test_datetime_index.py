...
def test_resample_timegrouper2(dates, unit):
    ...
    expected = DataFrame(
        {"B": [1, 0, 2, 2, 1], "C": [1, 0, 2, 2, 1]}, index=exp_idx, columns=["B", "C"],
    )
    if df["A"].isna().any():
        expected.index = expected.index._with_freq(None)
    tm.assert_frame_equal(result, expected)
...
def test_resample_dst_anchor2(unit):
    ...
    tm.assert_frame_equal(
        result, expected, "W-MON Frequency",
    )
    ...
    tm.assert_frame_equal(
        result2, expected2, "2W-MON Frequency",
    )
    ...
    tm.assert_frame_equal(
        result3, expected3, "MS Frequency",
    )
    ...
    tm.assert_frame_equal(
        result4, expected4, "2MS Frequency",
    )
    ...
    tm.assert_frame_equal(
        result_d, expected_d, "D Frequency",
    )
...
def test_resample_aggregate_functions_min_count(func, unit):
    ...
    expected = Series(
        [np.nan], index=DatetimeIndex(["2020-03-31"], freq="QE-DEC").as_unit(unit),
    )
    tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize(
    "freq, freq_depr", [("2BME", "2BM"), ("2BQE", "2BQ"), ("2BQE-MAR", "2BQ-MAR"),],
)
...
def test_resample_ms_closed_right(unit):
    ...
    expected = DataFrame(
        {"ts": [datetime(2020, 2, 1), datetime(2020, 2, 4, 3, 59)]}, index=exp_dti,
    ).astype(f"M8[{unit}]")
    tm.assert_frame_equal(result, expected)
