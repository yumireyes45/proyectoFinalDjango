...
def test_groupby_resample_with_list_of_keys():
    ...
    expected = DataFrame(data={"val": [4.0, 3.5, 6.5, 3.0],}, index=mi_exp,)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_resample_on_index_with_list_of_keys():
    # GH 50840
    df = DataFrame(
        data={"group": [0, 0, 0, 0, 1, 1, 1, 1], "val": [3, 1, 4, 1, 5, 9, 2, 6],},
...
def test_groupby_resample_on_index_with_list_of_keys_multi_columns():
    ...
    expected = DataFrame(
        data={"first_val": [2.0, 2.5, 7.0, 4.0], "second_val": [4.5, 4.5, 5.0, 4.5],},
        index=mi_exp,
    )
    tm.assert_frame_equal(result, expected)
...
def test_groupby_resample_on_index_with_list_of_keys_missing_column():
    ...
    df = DataFrame(
        data={"group": [0, 0, 0, 0, 1, 1, 1, 1], "val": [3, 1, 4, 1, 5, 9, 2, 6],},
        index=Series(date_range(start="2016-01-01", periods=8), name="date",),
    )
    gb = df.groupby("group")
    rs = gb.resample("2D")
...
def test_groupby_resample_kind(kind):
    ...
    expected_index = pd.MultiIndex.from_product(
        [["A", "B"], dt_level], names=["group", "datetime"],
    )
