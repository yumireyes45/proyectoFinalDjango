    ...
    df_expected = DataFrame(
        {
            ...
            "答": [
                "".join(["答  邪既無量 正亦多途  大略為言不出二種 謂", "有得與無得 有得是邪須破 無得是正須申\n\t\t故",]),
                None,
...
def test_elem_and_attrs_only(kml_cta_rail_lines, parser):
    with pytest.raises(
        ValueError, match=("Either element or attributes can be parsed not both"),
    ):
...
def test_empty_attrs_only(parser):
    ...
    with pytest.raises(
        ValueError, match=("xpath does not return any nodes or attributes"),
    ):
...
def test_wrong_stylesheet(kml_cta_rail_lines, xml_data_path):
    ...
    with pytest.raises(
        xml_etree.XMLSyntaxError, match=("Start tag expected, '<' not found"),
    ):
        read_xml(kml_cta_rail_lines, stylesheet=xsl)
