...
def test_wrong_file_path(parser, geom_df):
    ...
    with pytest.raises(
        OSError, match=(r"Cannot save file into a non-existent directory: .*path"),
    ):
...
<data>
  ...
  <row shape="triangle">
    ...
    output = geom_df.to_xml(
        index=False, elem_cols=["degrees", "sides"], attr_cols=["shape"], parser=parser,
    )
    output = equalize_decl(output)
...
def test_stylesheet_wrong_path(geom_df):
    ...
    with pytest.raises(
        lxml_etree.XMLSyntaxError, match=("Start tag expected, '<' not found"),
    ):
