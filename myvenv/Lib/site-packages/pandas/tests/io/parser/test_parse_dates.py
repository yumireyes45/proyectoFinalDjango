...
def test_nat_parse(all_parsers):
    ...
    parser = all_parsers
    df = DataFrame(
        {"A": np.arange(10, dtype="float64"), "B": Timestamp("20010101").as_unit("ns"),}
    )
    df.iloc[3:6, :] = np.nan
...
"""
    ...
    expected = DataFrame(
        ...
        index=MultiIndex.from_arrays(
            [datetimes, [126, 23, 13]], names=["datetime", "prn"],
        ),
    )
    tm.assert_frame_equal(result, expected)
...
def test_date_parser_and_names(all_parsers):
    ...
    result = parser.read_csv_check_warnings(
        warn, "Could not infer format", data, parse_dates=["B"], names=["B"],
    )
...
def test_date_parser_usecols_thousands(all_parsers):
    ...
    if parser.engine == "pyarrow":
        ...
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(
                StringIO(data), parse_dates=[1], usecols=[1, 2], thousands="-",
            )
        return
...
2000-01-01 00:00:01,1"""
    ...
    expected = DataFrame(
        {
            "a": [Timestamp("2000-01-01 00:00:00"), Timestamp("2000-01-01 00:00:01"),],
            "b": 1,
        }
    )
    tm.assert_frame_equal(result, expected)
