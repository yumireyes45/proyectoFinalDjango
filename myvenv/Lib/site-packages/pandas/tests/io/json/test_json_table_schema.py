...
class TestTableSchemaType:
    ...
    @pytest.mark.parametrize(
        "date_dtype",
        [np.dtype("<M8[ns]"), PeriodDtype("D"), DatetimeTZDtype("ns", "US/Central"),],
    )
...
class TestTableOrientReader:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pd.Index(range(4)),
            pd.date_range("2020-08-30", freq="d", periods=4,)._with_freq(None),
            pd.date_range(
    ...
    def test_read_json_table_orient_period_depr_freq(self, freq, recwarn):
        # GH#9586
        df = DataFrame(
            {"ints": [1, 2]}, index=pd.PeriodIndex(["2020-01", "2021-06"], freq=freq),
        )
        out = df.to_json(orient="table")
        result = pd.read_json(out, orient="table")
        tm.assert_frame_equal(df, result)
