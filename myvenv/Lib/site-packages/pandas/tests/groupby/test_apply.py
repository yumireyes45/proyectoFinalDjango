...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda x: dict(enumerate(x.index.to_list())), [{0: 0, 1: 1}, {0: 2, 1: 3}],),
        (
...
@pytest.mark.parametrize(
    "udf", [(lambda x: x.copy()), (lambda x: x.copy().rename(lambda y: y + 1))],
)
@pytest.mark.parametrize("group_keys", [True, False])
...
def test_apply_is_unchanged_when_other_methods_are_called_first(reduction_func):
    ...
    expected = DataFrame(
        {"b": [15, 6], "c": [150, 60]}, index=Index([88, 99], name="a"),
    )
...
def test_apply_dropna_with_indexed_same(dropna):
    ...
    # GH#43205
    df = DataFrame(
        {"col": [1, 2, 3, 4, 5], "group": ["a", np.nan, np.nan, "b", "b"],},
        index=list("xxyxz"),
    )
    msg = "DataFrameGroupBy.apply operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
...
def test_apply_index_key_error_bug(index_values):
    # GH 44310
    result = DataFrame(
        {"a": ["aa", "a2", "a3"], "b": [1, 2, 3],}, index=Index(index_values),
    )
    expected = DataFrame(
        {"b_mean": [2.0, 3.0, 1.0],}, index=Index(["a2", "a3", "aa"], name="a"),
    )
    msg = "DataFrameGroupBy.apply operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
