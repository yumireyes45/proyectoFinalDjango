...
def groupsort_indexer(
    index: np.ndarray, ngroups: int,  # const int64_t[:]
) -> tuple[
    np.ndarray, np.ndarray,  # ndarray[int64_t, ndim=1]  # ndarray[int64_t, ndim=1]
]: ...
def kth_smallest(arr: np.ndarray, k: int,) -> Any: ...  # numeric[:]  # numeric
...
def nancorr_spearman(
    mat: npt.NDArray[np.float64], minp: int = ...,  # ndarray[float64_t, ndim=2]
) -> npt.NDArray[np.float64]: ...  # ndarray[float64_t, ndim=2]
...
def get_fill_indexer(
    mask: npt.NDArray[np.bool_], limit: int | None = None,
) -> npt.NDArray[np.intp]: ...
def pad(
...
def is_monotonic(
    arr: np.ndarray, timelike: bool,  # ndarray[numeric_object_t, ndim=1]
